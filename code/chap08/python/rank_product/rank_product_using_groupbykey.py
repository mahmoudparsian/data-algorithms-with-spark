...
def compute_mean(input_path):
   ...
   genes_grouped = genes.groupByKey()
        
   print("genes_grouped", genes_grouped.collect())
   #now compute the mean per gene
   genes_mean = genes_grouped.mapValues(lambda v: float(sum(v))/float(len(v)))
...
def compute_ranked_products(ranks):
    ...
    # grouped_by_gene: RDD[(String, Iterable<Double>)]
    grouped_by_gene = union_rdd.groupByKey()
                     
    # next calculate ranked products and the number of elements
    ranked_products = grouped_by_gene.mapValues(
        lambda v : (pow(math.prod(v), float(len(v))), len(v))
    )
...
# ranked_products  : RDD[(mapped_id, (ranked_product, N))]
