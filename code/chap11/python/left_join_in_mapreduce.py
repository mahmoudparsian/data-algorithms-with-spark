...
left_join(R, S): R is a left table and S is a right 
table. The LEFT JOIN(R, S)  returns all rows from 
the left table (R), and the matching rows from the 
right table (S) . The result is NULL from the right 
...
Therefore, to perform a left join between two relations 
...
reduce(key, values) {
  ...
  # if there is no R elements: means that key belongs to S
  if (len(list_R) == 0): return []
  
  if (len(list_S) == 0): list_S.append(None)
...
def left_join(entry):
  ...
  # if there is no T1 elements: means that key belongs to T2
  if (len(T1) == 0): return []
  
  if (len(T2) == 0): T2.append(None)
