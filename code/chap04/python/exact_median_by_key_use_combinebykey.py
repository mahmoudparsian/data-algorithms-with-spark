...
#  target_rdd = source_rdd.combineByKey(init_function, seq_function, comb_function) 
...
def main():
    ...
    # combined_by_key: RDD[(String, [Integer])]
    combined_by_key = rdd.combineByKey(lambda v : [v], \
                                       lambda L, v: L.append(v), \
                                       lambda C, D: C+D)

    # show combined_by_key
    print("combined_by_key = ", combined_by_key.mapValues(lambda values: list(values)).collect())
    # [
    #  ('k3', [10, 12, 30, 32]),
    #  ('k2', [6, 7, 8, 1, 2]),
    #  ('k1', [1, 2, 3, 4, 5])
    ...
    # find median per key 
    median_per_key = combined_by_key.mapValues(statistics.median)
...
$SPARK_HOME/bin/spark-submit exact_median_by_key_use_combinebykey.py
combined_by_key =  
[
 ('k1', [1, 2, 3, 4, 5]), 
 ('k3', [10, 12, 30, 32]), 
