 ...
 *  targetRdd = sourceRdd.combineByKey(initFn, seqFunction, combFunction)
 *  where init_function creates a combined data type as C
 *
 ...
 *
 */
object ExactMedianByKeyUseCombineByKey {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // combinedByKey: RDD[(String, [Integer])]
    val combinedByKey = rdd.combineByKey(v => Seq(v),
      (x: Seq[Int], y: Int) => x.appended(y),
      (x: Seq[Int], y: Seq[Int]) => x.appendedAll(y))

    // show combinedByKey
    println("combinedByKey = " + combinedByKey.collect().mkString("Array(", ", ", ")"))
    // [
    //  ('k3', [10, 12, 30, 32]),
    //  ('k2', [6, 7, 8, 1, 2]),
    //  ('k1', [1, 2, 3, 4, 5])
    ...
    // find median per key 
    val medianPerKey = combinedByKey.mapValues(breeze.stats.median(_))
...
/*
combinedByKey = Array((k3,List(10, 12, 30, 32)), (k1,List(1, 2, 3, 4, 5)), (k2,List(1, 2, 6, 7, 8)))
