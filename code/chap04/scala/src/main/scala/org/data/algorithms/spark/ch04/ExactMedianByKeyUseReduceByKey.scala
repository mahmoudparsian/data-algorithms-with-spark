...
object ExactMedianByKeyUseReduceByKey {
  def main(args: Array[String]): Unit = {
    ...
    // listRdd: RDD[(String, [Integer])]
    val listRdd = rdd.mapValues(Seq(_))

    // reduce values by key
    // listRdd: RDD[(String, [Integer])]
    // reducedByKey: RDD[(String, [Integer])]
    val reducedByKey = listRdd.reduceByKey((a, b) => a ++ b)

    // show reducedByKey
    println("reducedByKey = " + reducedByKey.collect().mkString("Array(", ", ", ")"))
    // [
    //  ('k3', [10, 12, 30, 32]),
    //  ('k2', [6, 7, 8, 1, 2]),
    //  ('k1', [1, 2, 3, 4, 5])
    ...
    // find median per key
    val medianPerKey = reducedByKey.mapValues(breeze.stats.median(_))
...
/*
reducedByKey = Array((k3,List(10, 12, 30, 32)), (k1,List(1, 2, 3, 4, 5)), (k2,List(1, 2, 6, 7, 8)))
