...
#end-def
#---------------------------------------
# lambda 1 for combineByKey()
# create a single element dictionary with counter of 1
def create_combiner(v):
    return Counter([v])
#end-def
#---------------------------------------
# lambda 2 for combineByKey()
# merge a single element (v) into a dictionary (C) 
def merge_value(C, v):
    C[v] += 1
    return C
#end-def
#---------------------------------------
# lambda 3 for combineByKey()
# combine dictionaries C and D into a single dictionary 
def merge_combiners(C, D):
    return C + D
#end-def
#--------------------------------------- 
#---------------------------------------          
...
anagrams_list = rdd_key_value.combineByKey(create_combiner, merge_value, merge_combiners)
