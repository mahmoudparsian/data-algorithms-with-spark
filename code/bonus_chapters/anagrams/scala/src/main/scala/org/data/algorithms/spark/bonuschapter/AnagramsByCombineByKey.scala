...
object AnagramsByCombineByKey {
  ...
  def createCombiner: String => List[String] = V => List(V);

  def mergeValue: (List[String], String) => List[String] =
    (C, v) => C ++ List(v)

  def mergeCombiners: (List[String], List[String]) => List[String] =
    (C1, C2) => C1 ++ C2
  ...
  def main(args: Array[String]): Unit = {
    ...
    val anagramsList = rddKeyValue.combineByKey(createCombiner, mergeValue, mergeCombiners)
    println("anagramsList=" + anagramsList.mapValues(l => l).collect().mkString("[", ", ", "]"))

    // anagrams: RDD[(K, Map<word, Integer>)]
    val anagrams = anagramsList.mapValues(listToDict)
