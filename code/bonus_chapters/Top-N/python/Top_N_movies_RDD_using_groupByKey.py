...
# Sort the values using sortBy() using values obtained from 'grouped'
sorted = grouped.sortBy(lambda x: x[1], ascending=False)  
# Print the top-10 values to output
# top10 = [(movie_id, frequency)]
# top N
topN_movies = sorted.take(N)
print("topN_movies=", topN_movies)

# now, find name of movies (by using movie_id)
 
# creating a Python data frame
# Note: since the number of movies is not 
#       a big data, we create a Python DataFrame 
